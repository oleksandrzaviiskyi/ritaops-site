...
| `/join/[code]` | Организатор (короткая ссылка) | Редирект на портал; `code = btoa(slug:token).slice(0,12)` |
| `/guest/[groupSlug]` | Гость | Анкета `guestSubmission` в Sanity |
| `/api/guest` | API гостевой формы | GET — инфо о группе; POST — отправка анкеты |
