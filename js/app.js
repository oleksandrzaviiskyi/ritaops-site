...
(function () {
    ...
    if (placedCard) {
      ...
      if (placedCard) {
        ...
        field.querySelectorAll('.card').forEach(function(c) {
          ...
          const h = c.offsetHeight || 250
          if (t + h + 16 > maxBottom) maxBottom = t + h + 16
        })
        placedCard.style.left = '20px'
        placedCard.style.top = maxBottom + 'px'
        field.style.minHeight = (maxBottom + 300) + 'px'
      }
    }
  ...
  // Global delegated drag — cards are absolute in field canvas
  ;(function initGlobalDrag() {
    let dragging = false, dragCard = null, ox = 0, oy = 0
    ...
    field.addEventListener('mousedown', function (e) {
      ...
      if (!dragCard) return
      // ox/oy = offset of click inside card (field-relative)
    ...
    document.addEventListener('mousemove', function (e) {
      ...
      let nx = e.clientX - fieldRect.left - ox
      let ny = e.clientY - fieldRect.top + scrollTop - oy
      nx = Math.max(0, Math.min(nx, window.innerWidth - panelW - dragCard.offsetWidth - 8))
      ny = Math.max(0, ny)
      dragCard.style.left = nx + 'px'
      dragCard.style.top = ny + 'px'
      // expand canvas if needed
      const bottom = ny + dragCard.offsetHeight + 40
      if (bottom > field.offsetHeight) field.style.minHeight = bottom + 'px'
    })
...
})().card{background:var(--surface);border:1px solid var(--line);border-radius:20px;padding:16px 16px 13px;
  backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);
  filter:drop-shadow(0 2px 16px rgba(40,30,18,.07));
  animation:rise .36s cubic-bezier(.2,.7,.2,1) both;
  position:absolute;overflow:hidden;z-index:12;width:300px;
